...
import { AccountType, DepositWithdrawType, SignupType } from '../features/User/User.types';
...
const API = createApi({
  ...
  endpoints: (build) => ({
    ...
    deposit: build.mutation<DepositWithdrawType, { amount: number }>({
      query: (body) => ({ url: '/auth/deposit', method: 'POST', body }),
    }),
    withdraw: build.mutation<DepositWithdrawType, { amount: number }>({
