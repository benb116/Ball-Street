...
async function fillOffers(bidid, askid, price) {
  ...
  const out = sequelize.transaction(isoOption, async (t) => attemptFill(t, bidid, askid, price))
    .catch((err) => {
      switch (err) {
        case 'bid':
          console.log('bid err');
...
async function attemptFill(t, bidid, askid, price) {
  ...
  const tprice = price || (boffer.createdAt < aoffer.createdAt ? boffer.price : aoffer.price);
